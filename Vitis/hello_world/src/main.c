...
void check_output(int ***opsum_data, int ***opsum_data_golden) {
  int i, j, k;
  for (i = 0; i < e; i++) {
    for (j = 0; j < OFMAP_COL; j++) {
      for (k = 0; k < p * t; k++) {
        if (opsum_data[i][j][k] != opsum_data_golden[i][j][k]) {
          printf("opsum[%d][%d][%d]: your result = %6d, expect = %6d\n", i, j,
                 k, opsum_data[i][j][k], opsum_data_golden[i][j][k]);
        } else {
          printf("opsum[%d][%d][%d]: pass\n", i, j, k);
        }
      }
    }
  }
...
int main() {
  ...
  check_output(opsum, opsum_golden);
  cleanup_platform();
  return 0;
}
